...
class CNNLSTMIDSNode:
    ...
    def _setup_logging(self):
        ...
        try:
            ...
            log_file = os.path.join(log_dir, f"cnn_lstm_ids_{timestamp}.log")
            ...
            self.ids_logger = logging.getLogger('CNN_LSTM_IDS')
            self.ids_logger.setLevel(logging.INFO)
            
            rospy.loginfo(f"📝 Logging to: {log_file}")
            self.ids_logger.info("🚀 CNN+LSTM IDS Node logging initialized")
    ...
    def _get_model_directory(self):
        ...
        possible_paths = [
            "/home/jakelcj/ids_ws/src/ros_ids/models/cnnlstm2",
            "/home/jakelcj/ids_ws/src/ros_ids/models/cnnlstm2",
            "~/ids_ws/src/ros_ids/models/cnnlstm2",
            "./cnnlstm2"
        ]
        ...
        default_path = "/home/jakelcj/ids_ws/src/ros_ids/models/cnnlstm2"
    ...
    def _prepare_features(self, df):
        ...
        try:
            ...
            if len(matched_features) >= len(self.feature_names) * 0.8:  # At least 80% match
                ...
                X = pd.DataFrame(index=df_clean.index, columns=self.feature_names, dtype=float)
    ...
    def _apply_complete_pipeline(self, X):
        ...
        try:
            ...
            # Step 6: Fuse PCA features with KNN and RF probabilities (FIXED!)
            fused_features = np.concatenate([
                pca_features,    # 16 PCA features
                knn_proba,       # 8 KNN probabilities  
                rf_proba         # 8 RF probabilities
            ], axis=1)           # Total: 32 features
    ...
    def _process_csv_file(self, file_path):
        ...
        try:
            ...
            # Prepare features (remove label columns if present)
            feature_df = new_rows.copy()
