...
class CNNMHAIDSNode:
    ...
    def _setup_logging(self):
        ...
        try:
            ...
            log_dir = os.path.join(os.path.expanduser("~"), "ids_logs")
            os.makedirs(log_dir, exist_ok=True)
            
            # Setup log file path with timestamp
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            log_file = os.path.join(log_dir, f"cnn_mha_ids_{timestamp}.log")
            
            # Create file handler with rotation (max 10MB, keep 5 files)
            ...
            # Add to root logger and ROS logger
            logging.getLogger().addHandler(self.file_handler)
            logging.getLogger().setLevel(logging.INFO)
            
            # Create IDS-specific logger
            self.ids_logger = logging.getLogger('CNN_MHA_IDS')
            self.ids_logger.setLevel(logging.INFO)
            
            rospy.loginfo(f"📝 Logging to: {log_file}")
            self.ids_logger.info("🚀 CNN+MHA IDS Node logging initialized")
            self.ids_logger.info(f"📁 Model directory: {self.model_dir}")
            self.ids_logger.info(f"📂 CSV directory: {self.csv_dir}")
            
        except Exception as e:
            rospy.logwarn(f"⚠️ Failed to setup file logging: {str(e)}")
            self.ids_logger = None
    
    def _get_model_directory(self):
        """Get the model directory path"""
        # Try different possible locations
        possible_paths = [
            "/home/jakelcj/ids_ws/src/ros_ids/models/cnnmha",
            "~/ids_ws/src/ros_ids/models/cnnmha", 
            "./models/cnnmha",
            rospy.get_param('~model_path', '/home/jakelcj/ids_ws/src/ros_ids/models/cnnmha')
        ]
    ...
    def _apply_complete_pipeline(self, X):
        ...
        try:
            ...
            # Step 6: Fuse KNN and RF probabilities only (CNN+MHA approach)
            # Note: CNN+MHA models are trained WITHOUT PCA features in final fusion
            combined_features = np.concatenate([
                knn_proba,       # 8 KNN probabilities
                rf_proba         # 8 RF probabilities  
            ], axis=1)           # Total: 16 features
    ...
    def _prepare_features(self, df):
        """Prepare features from dataframe with intelligent column mapping"""
        try:
            rospy.loginfo(f"📊 Input CSV shape: {df.shape}")
            rospy.loginfo(f"🎯 Model expects {len(self.feature_names)} features")
            
            # Remove common non-feature columns
            exclude_cols = ['Label', 'label', 'Flow ID', 'Src IP', 'Dst IP', 'Protocol', 'Timestamp']
            df_clean = df.drop(columns=[col for col in exclude_cols if col in df.columns], errors='ignore')
            
            rospy.loginfo(f"📊 After removing non-feature columns: {df_clean.shape}")
            ...
            # Try exact column matching first
            matched_features = []
            missing_features = []
            ...
            rospy.loginfo(f"✅ Exact matches: {len(matched_features)}/{len(self.feature_names)}")
            
            # If we have exact matches, use them
            if len(matched_features) >= len(self.feature_names) * 0.8:  # At least 80% match
                # Create feature matrix with expected features
                X = pd.DataFrame(index=df_mapped.index, columns=self.feature_names, dtype=float)
                
                # Fill matched features
                ...
                rospy.loginfo(f"📊 Using exact matching: {len(matched_features)} matched, {len(missing_features)} filled with zeros")
                
            else:
                # Fallback: Use first N columns if they're numeric
                rospy.logwarn(f"⚠️ Low feature match rate. Trying positional mapping...")
                
                # Get numeric columns only
                numeric_df = df_mapped.select_dtypes(include=[np.number])
                
                if len(numeric_df.columns) >= len(self.feature_names):
                    # Take first N numeric columns
                    selected_cols = numeric_df.columns[:len(self.feature_names)]
                    X = pd.DataFrame(numeric_df[selected_cols].values, 
                                   columns=self.feature_names, 
                                   index=df_mapped.index)
                    rospy.loginfo(f"📊 Using positional mapping: first {len(self.feature_names)} numeric columns")
                    
                else:
                    rospy.logerr(f"❌ Insufficient numeric columns: got {len(numeric_df.columns)}, need {len(self.feature_names)}")
                    return None
            ...
            rospy.loginfo(f"✅ Final feature matrix: {X.shape}")
            return X.values
    ...
    def _process_csv_file(self, file_path):
        ...
        try:
            ...
            if self.ids_logger:
                self.ids_logger.info(f"Processing {len(new_rows)} new rows from {os.path.basename(file_path)}")
            ...
            for i, (pred, proba) in enumerate(zip(predictions, probabilities)):
                ...
                if pred != 0:  # Not normal traffic
                    ...
                    if self.ids_logger:
                        self.ids_logger.warning(f"ATTACK DETECTED: {attack_class} | Confidence: {confidence:.3f} | Row: {start_idx + i} | File: {os.path.basename(file_path)}")
                        self.ids_logger.info(f"Attack probabilities: {[f'{p:.3f}' for p in proba]}")
            ...
            if self.ids_logger:
                self.ids_logger.info(f"Successfully processed {len(new_rows)} rows. Total predictions: {self.total_predictions}, Attacks: {self.attack_detections}")
            
        except Exception as e:
            rospy.logerr(f"❌ Error processing {file_path}: {str(e)}")
            if self.ids_logger:
                self.ids_logger.error(f"Error processing {file_path}: {str(e)}")
            import traceback
            rospy.logerr(traceback.format_exc())
    ...
    def _print_statistics(self):
        ...
        # Log statistics to file every 60 seconds
        if self.ids_logger:
            self.ids_logger.info(f"STATS | Uptime: {uptime:.1f}s | Predictions: {self.total_predictions} | Attacks: {self.attack_detections} | Rate: {attack_rate:.2f}% | Files: {len(self.processed_files)}")
...
def main():
    ...
    try:
        ...
        print(f"⚙️ Node: cnn_mha_ids_node_linux.py")
        print("="*60 + "\n")
