...
CSV_FILE_PATH = "/home/jakelcj/output.csv"  # UPDATE THIS PATH
...
def format_prediction_message(predictions, pipeline_info):
    ...
    try:
        ...
        rospy.logdebug(f"Available class mappings: {class_names}")
...
class CNNLSTMIDSNode:
    ...
    def run(self):
        ...
        """
        try:
            # Process the CSV data once
            self.process_csv_data()
            
            # Keep node alive for continuous monitoring
            rospy.loginfo("🔄 Entering monitoring mode...")
